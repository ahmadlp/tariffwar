...
%TARIFFWAR.VIZ.EXPORT_WELFARE_MAP  Export a static welfare choropleth.
%
%   tariffwar.viz.export_welfare_map(countries, pct_change, ...
%       'output_file', 'results/maps/welfare_map_wiod_2014_IS.png')
    ...
    if isempty(strtrim(output_file))
        error('tariffwar:viz:missingOutputFile', ...
    ...
    if numel(countries) ~= numel(pct_change)
        error('tariffwar:viz:sizeMismatch', ...
            'countries and pct_change must have the same length.');
    end

    world = tariffwar.viz.load_world_polygons();
    out_dir = fileparts(output_file);
    if ~isempty(out_dir) && ~isfolder(out_dir)
        mkdir(out_dir);
    end

    [value_map, missing_codes] = build_value_map(countries, pct_change, world.features);
    if ~isempty(missing_codes)
        fprintf('Warning: Omitting countries without map geometry: %s\n', ...
            strjoin(missing_codes, ', '));
    end
    ...
    annotation(fig, 'textbox', [0.05, 0.885, 0.9, 0.04], ...
        'String', 'Percentage change in welfare under a Nash tariff equilibrium.', ...
        'EdgeColor', 'none', ...
        'FontName', 'Helvetica', ...
        'FontSize', 11, ...
        'Color', [0.2, 0.2, 0.2], ...
    ...
    run_label = build_run_label(p.Results.dataset, p.Results.year, p.Results.elasticity);
...
function [value_map, missing_codes] = build_value_map(countries, pct_change, features)
    ...
    for i = 1:numel(countries)
        ...
        if ~ismember(code, iso_codes)
            missing_codes{end + 1} = code; %#ok<AGROW>
        end
    end

    missing_codes = unique(missing_codes, 'stable');
end
...
function label = build_run_label(dataset, year, elasticity)
    parts = {};
