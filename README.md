...
|-- results/                     Analysis output CSV (generated by run.m)
|
...
2. **Build** `.mat` files from raw CSVs (`tariffwar.build_all`)
3. **Analyze** all datasets x years x elasticities (`tariffwar.run`)
...
```
raw CSV --> build_cubes_{wiod,icio,itpd}.m --> build_all.m --> mat/*.mat
                                                                  |
                                                              load_data.m
                                                                  |
            run.m <-- balance_trade.m <-- compute_derived_cubes.m |
              |                                                    |
...
To investigate, build ITPD cubes via `tariffwar.build_all('dataset', 'itpd', 'years', 2019)` and use the resulting `mat/ITPD2019.mat` with `tariffwar.solver.trade_war_baseline` directly.
