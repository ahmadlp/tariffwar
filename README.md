...
```
+tariffwar/
|-- main.m                       One-click runner: tariffwar.main
|-- run.m                        Analysis entry point: tariffwar.run('wiod', 2014, 'L21')
|-- defaults.m                   Solver defaults and paths
|-- build_all.m                  Data construction (run once to build mat/ files)
|-- .gitignore                   Ignores raw_data/ and results/
|-- mat/                         Prebuilt .mat files (COMMITTED — analysis works immediately)
|-- raw_data/                    Downloaded external data (GITIGNORED — only needed for rebuild)
|-- results/                     Analysis output CSV (GITIGNORED — generated by run.m)
|
|-- +solver/                     Nash equilibrium solver
|   |-- nash_equilibrium.m       Solves 3N-unknown system via fsolve
|   |-- trade_war_baseline.m     System of equations (Eqs 6, 7, 14)
|   |-- balanced_trade_baseline.m  Balanced-trade (D=0) system of equations
|   |-- solver_options.m         Builds optimoptions struct from config
|
|-- +welfare/                    Welfare computation
|   |-- compute_gains.m          Wrapper: percent welfare changes from X_sol
|   |-- welfare_gains_baseline.m Core hat-algebra welfare calculation
|
|-- +data/                       Trade balancing, derived cubes, data preparation
|   |-- balance_trade.m          DEK balanced-trade exercise (fsolve, 2N unknowns)
|   |-- compute_derived_cubes.m  Builds lambda, Yi, Ri, e_ik from trade cubes
|   |-- prepare_wiod.m           Full WIOD pipeline (used by build_all)
|   |-- prepare_icio.m           OECD ICIO Extended pipeline
|   |-- prepare_itpd.m           USITC ITPD-S pipeline
|
|-- +io/                         File I/O
|   |-- load_data.m              Loads prebuilt .mat files from mat/
|   |-- load_tariffs.m           Loads NxNxS tariff cube
|   |-- load_gdp.m               Loads real GDP data
|   |-- load_country_list.m      Loads country names
|   |-- download_*.m             Dataset downloaders
|
|-- +elasticity/                 Trade elasticity sources
|   |-- registry.m               Master registry (8 sources)
|   |-- get.m                    Returns Sx1 elasticity vector
|   |-- get_sigma_cube.m         Builds NxNxS CES sigma cube
|   |-- +sources/                Individual source implementations
|
|-- +concordance/                Sector-mapping tables
|-- +tariff/                     Tariff data readers
|-- +results/precomputed/        Pre-solved result .mat files
|
|-- +test/                       Test suite: tariffwar.test.run_all()
    |-- run_all.m                Runs all tests
    |-- test_config.m            Defaults tests
    |-- test_data_loading.m      Data loading tests
    |-- test_elasticity.m        Elasticity module tests
    |-- test_concordance.m       Concordance tests
    |-- test_cubes.m             Cube computation tests
    |-- test_solver_smoke.m      Full solver smoke test (WIOD 2014)
```
...
tariffwar.main            % run all datasets x years x elasticities
```

**To rebuild `.mat` files from raw data** (only needed if modifying the data pipeline):

```matlab
% Step 1: Download raw data into +tariffwar/raw_data/
tariffwar.io.download_all()     % ~2 GB total; requires internet

% Step 2: Build .mat files from raw data (~30 min)
tariffwar.build_all()
```

The `raw_data/` and `results/` directories are gitignored.
...
```
raw CSV/mat --> prepare_{wiod,icio,itpd}.m --> balance_trade.m --> run.m
                    |                              |                |
              N, S, Xjik_3D                  balanced Xjik    lambda, Yi, Ri, e
              tjik_3D                                              |
                                                           nash_equilibrium.m
                                                                   |
                                                            X_sol (3N vector)
                                                                   |
                                                        welfare_gains_baseline.m
                                                                   |
                                                          welfare_pct (N x 1)
```
...
- **Notes:** 50 raw sectors. 27 goods sectors (ISIC sections A, B, C) are preserved individually; 23 services sectors (D through T) are collapsed to 1 aggregate. Final demand columns are identified by suffix matching (HFCE, NPISH, GGFC, GFCF, INVNT, DPABR). Only 81 of 85 economies have FD columns.
...
| `caliendo_parro_2015` | Caliendo & Parro (2015, ReStud) | 40 (ISIC Rev 3) | Stub |
| `bagwell_staiger_yurukoglu_2021` | BSY (2021, Econometrica) | HS-based | Stub |
| `giri_yi_yilmazkuday_2021` | GYY (2021, JIE) | 20 (OECD) | Stub |
| `shapiro_2016` | Shapiro (2016, AEJ) | 21 (HS sections) | Stub |
| `fontagne_2022` | Fontagne et al. (2022, JIE) | ~5000 (HS6) | Stub |
| `lashkaripour_lugovskyy_2023` | LL (2023, AER) | 16 (WIOD) | Stub |
...
To run everything at once: `tariffwar.main` (data construction is commented out; only the analysis runs).

Three positional args: `(datasets, years, elasticities)`. All accept scalar or array/cell.
...
|--------|---------|-------------|
| `'Algorithm'` | `'trust-region-dogleg'` | fsolve algorithm |
| `'MaxIter'` | `Inf` | Max solver iterations |
| `'TolFun'` | `1e-12` | Function tolerance |
...
| File | Description |
|------|-------------|
| `wiod_lashkaripour_2021.mat` | WIOD 2000-2014, 15 years of results |
| `wiod_uniform_4.mat` | WIOD 2000-2014 with uniform sigma=4 |
| `icio_lashkaripour_2021.mat` | ICIO 2019, single year |
| `icio_uniform_4.mat` | ICIO 2019, single year |
| `itpd_uniform_4.mat` | ITPD-S 2019, N=135, S=154, uniform sigma=4 |
| `itpd_2019_cubes_S154.mat` | ITPD-S 2019 raw data cubes (N=135, S=154) for debugging |
| `itpd_2019_cubes_S16.mat` | ITPD-S 2019 data cubes collapsed to 16 sectors |

The cube files (`itpd_2019_cubes_*.mat`) contain: `N`, `S`, `Xjik_3D`, `tjik_3D`, `lambda_jik3D`, `Yi3D`, `Ri3D`, `e_ik3D`, `sigma_k3D_lash`, `sigma_k3D_u4`, `countries`. These can be loaded directly and passed to `tariffwar.solver.trade_war_baseline` for debugging.
...
### 3. Choosing Levenberg-Marquardt over Trust-Region-Dogleg
...
**The fix:** Switched to `trust-region-dogleg`, which is Matlab's default for `fsolve`. Despite having the same per-iteration cost (both require full Jacobian), trust-region-dogleg converges in far fewer iterations for this problem class: 4-8 iterations vs 200+.

**Lesson:** `trust-region-dogleg` is generally better for square nonlinear systems (N equations, N unknowns). `levenberg-marquardt` is designed for overdetermined systems (more equations than unknowns) and adds unnecessary damping for square systems. The original code used LM because it happened to work for the small WIOD problem, but it is not the right choice for larger problems.
...
1. **`max(denominator, eps)` guards** in all division operations (`balance_trade.m`, `run.m`, `trade_war_baseline.m`, `welfare_gains_baseline.m`). When the denominator is 0, the result is `0/eps = 0`, which is the correct economic interpretation (zero trade share).
2. **Trade floor** in `prepare_itpd.m`: `Xijs3D = max(Xijs3D, mean(positive_values) * 1e-12)`. This fills zero entries with a negligibly small positive value, ensuring all denominators are strictly positive.
...
To facilitate further investigation, two data cube files have been saved:

- `itpd_2019_cubes_S154.mat` (45 MB): Full N=135, S=154 cubes with both elasticity sets
- `itpd_2019_cubes_S16.mat` (5.5 MB): Collapsed N=135, S=16 cubes

These can be loaded and passed directly to `trade_war_baseline` for solver experimentation:

```matlab
load('+tariffwar/+results/precomputed/itpd_2019_cubes_S16.mat');
% Variables: N, S, Xjik_3D, tjik_3D, lambda_jik3D, Yi3D, Ri3D,
%            e_ik3D, sigma_k3D_lash, sigma_k3D_u4, countries

% Test with uniform_4 (should converge):
T0 = [1.1*ones(N,1); 1.1*ones(N,1); 1.2*ones(N,1)];
opts = optimoptions('fsolve', 'Display', 'iter', 'Algorithm', 'trust-region-dogleg');
target = @(X) tariffwar.solver.trade_war_baseline(X, N, S, Yi3D, Ri3D, ...
    e_ik3D, sigma_k3D_u4, lambda_jik3D, tjik_3D);
[X_sol, ~, ef] = fsolve(target, T0, opts);  % Should converge

% Test with lashkaripour_2021 (will NOT converge):
target_lash = @(X) tariffwar.solver.trade_war_baseline(X, N, S, Yi3D, Ri3D, ...
    e_ik3D, sigma_k3D_lash, lambda_jik3D, tjik_3D);
[X_sol_lash, ~, ef_lash] = fsolve(target_lash, T0, opts);  % Will stall
```

### Possible Directions for Future Investigation

1. **Continuation method:** Solve first with uniform_4 (which converges), then use that solution as the initial guess for lashkaripour_2021. This provides a much better starting point than the default T0 = [1.1, 1.1, 1.2].

2. **Analytical Jacobian:** The current solver uses finite-difference Jacobian approximation (3N+1 function evaluations per iteration). Providing an analytical Jacobian would improve convergence by orders of magnitude for stiff systems. The Jacobian of `trade_war_baseline` can be derived from the CES demand structure.

3. **Sector-specific solver:** Instead of solving all 3N equations simultaneously, decompose the problem by sector or by country group to reduce conditioning issues.

4. **Proper zero-trade handling:** Replace the trade floor with explicit zero handling: if `lambda_jik = 0`, skip that term in all sums. This avoids creating tiny-but-nonzero Jacobian entries that degrade conditioning.

5. **ROW aggregation:** Currently, the ITPD-S country filter simply drops small countries. Their trade with retained countries is lost, distorting import shares. A proper implementation would aggregate dropped countries into a Rest-of-World composite, preserving total trade volumes.
...
```matlab
result = tariffwar.run('icio', 2019, 'U4')
```
...
```matlab
load('+tariffwar/+results/precomputed/wiod_lashkaripour_2021.mat');
```
