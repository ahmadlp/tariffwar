...
`tariffwar.rerun_failed()` reads the existing `results.csv`, identifies all rows with `exitflag <= 0`, and re-runs those (dataset, year, elasticity) triples. Results are written to a temporary CSV; improved rows (exitflag > 0) are merged back into the main CSV. The temp file is always cleaned up via `onCleanup`.

```matlab
tariffwar.rerun_failed()                          % re-run all failed cases
tariffwar.rerun_failed('datasets', {'wiod'})      % restrict to WIOD failures only
```
...
% Reads results.csv, re-runs failed triples, merges improvements back.
